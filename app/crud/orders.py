...
async def update_order(order_id, order):
    ...
    update_result = await Database.orders.update_one({"id": order_id}, {"$set": order.dict()})
...
async def delete_order(order_id):
    ...
    result = await Database.orders.delete_one({"id": order_id})
